...
YAML 1.2, when including the common
[JSON schema](https://yaml.org/spec/1.2/spec.html#id2803231)), supports *most* of CPS.
...
it, but other implementations may not, so do be careful with its use. A common workaround is
to encode byte arrays as Base64 strings, which is Compris's default serialization mode for
**bytes** in YAML.

By the way, YAML with the JSON schema is a superset of JSON. That means any YAML parser is also a
JSON parser. If you need to parse both and are trying to conserve code, you can disable JSON
parsing and simply treat JSON as YAML, though note that a dedicated JSON parser may be more
efficient.

Note that YAML is a notoriously ambiguous specification and you will find many inconsistencies
between parser implementations. Though the release of YAML 1.2 was an important step forward in
clarity, it unfortunately made necessary breaking changes. Many existing implementations added
support for 1.2 while retaining some 1.1 features, resulting in hybrid behavior that is even more
idiosyncratic.

The status quo is such that YAML is a poor choice for ensuring portability, whatever other
...
Firstly, it does not distinguish between number types. Parsers handle this challenge in a variety
of ways, from just assuming they are all floats (bad; very bad) to always encoding them as decimal
strings (wasteful). With Compris we opted to give you some control over how JSON numbers are
parsed.

Secondly, JSON map keys must be strings. By default Compris will stringify all JSON keys into
a JSON representation, which can then be parsed: JSON-in-JSON. However, Compris also has a
serialization mode that outputs maps as lists of key-value pairs, and can even do so conditionally,
...
JSON can be retrofitted to support all of CPS by introducing simple conventions. Indeed, there
have been attempts to extend JSON for similar purposes. For Compris, we've decided to introduce a
new one, where the design goals are 1) 100% wire-compatibility with JSON, and 2) support CPS;
nothing more and nothing less. We call it "XJSON" (eXtended JSON).
...
We allow this behavior because native JSON maps behave this way, at least in most implementations.
...
support `Hash` and does not have deterministic order.
...
Actually, Compris is a rewrite of Go [ARD](https://github.com/tliron/go-ard), which already
uses yamlkeys and has many of Compris's features.
