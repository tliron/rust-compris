...
3. OrderMap's less invasive side effect is that it retains insertion order, which might actually
   be useful when deterministic results are needed (e.g. in testing). Though note that CPS
   intentionally leaves map order undefined, so implementations should not rely on order.
