...
A Rust library to work with CPS (Composite Primitive Schema) data and parse it from and
serialize it to several binary and textual representation formats.
...
`#[derive(Resolve)]`. Our source code might help you get a grip on this challenging corner of Rust
programming.

Does Compris's resolve sound a bit like Serde deserialization? At its simplest, it can provide
the same results (and we do support Serde, too; see below). However, Compris's resolve is more
flexible in that it allows for accumulating errors (instead of failing on the first error, like
Serde), as well as configurable handling of nulls and undeclared fields. It even lets you to provide
your own custom context type.
...
"XJSON" serialization mode, which allows JSON to support all of CPS. Compris can, of course, also
parse and deserialize XJSON. Read more about XJSON [here](CPS.md).
