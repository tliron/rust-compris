...
The optional `resolve` feature makes it easy to "resolve" the normal value types into your
own types. This can be done recursively at any depth.

The API is simple but powerful, making use of a `#[derive(Resolve)]` procedural macro that
generates resolution code for you.

Sounds like Serde deserialization? It does have the same results, and we oo support Serde,
too (see below). However, Compris's resolution is much more flexible, as it allows accumulating
errors from nested resolution, instead of failing on the first error, like Serde. These errors
can be customized to support your own complex resolution rules and contain the location and
traversal path for where they ocurred, allowing for user-friendly error reports.
