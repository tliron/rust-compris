...
impl ValueBuilder {
    ...
    ///
    /// If there is no value, returns [Value::Empty].
    ///
    /// Panics if the builder is in an indeterminate state (e.g.
    /// [Self::end_container] wasn't called when necessary).
    ///
    ...
    /// Starts a list.
    ///
    /// Should be followed later by [Self::end_container].
    ...
    /// Ends a container.
    ///
    /// Follows either [Self::start_list] or [Self::start_map].
    pub fn end_container(&mut self) {
        _ = self.end_container_with_hints(false);
    }

    pub fn end_container_with_hints(&mut self, allow_hints: bool) -> Result<(), ReadError> {
            ...
            Some(mut value) => {
                ...
                match value {
                    Value::Map(ref mut map) => {
                        ...
                        if allow_hints {
                            if let Some(hinted_value) = to_hinted_value(map)? {
                                value = hinted_value;
                            }
                        }
                    }
