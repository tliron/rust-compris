...
pub fn generate_field_segment(field: &mut syn::Field, keys: &mut Vec<TokenStream>) -> syn::Result<TokenStream> {
        ...
        Some(name) => {
            ...
            let mut stream = quote! {
                ...
                if let Some(field) = self.get(key) {
                    if let Some(field) = ::compris::resolve::Resolve::resolve_for(field, error_reporter, ancestor)? {
                        resolved.#name = field;
                    }
                }
            };
                ...
                stream.extend(quote! {
                        ...
                        error_reporter.report(
                            ::compris::ToLocatableError::with_location(
                                ::compris::resolve::MissingRequiredKeyError::new(key), self, ancestor
                            )
                        )?;
                    }
                });
