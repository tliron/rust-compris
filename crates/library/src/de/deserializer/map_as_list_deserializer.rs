...
impl<'de> MapAsListDeserializer<'de> {
    ...
    fn next(&mut self) -> Result<(), DeserializationError> {
        let current_entry = self.iterator.next();

        if let Some(current_entry) = current_entry {
            if let Value::List(list) = current_entry {
                self.current_entry = list.to_couple();
            }

            return Err(DeserializationError::incompatible_value(current_entry));
        } else {
...
impl<'de> de::MapAccess<'de> for MapAsListDeserializer<'de> {
        ...
        SeedT: de::DeserializeSeed<'de>,
    {
        match self.current_entry {
            ...
            None => Err(DeserializationError::NoMoreItems), // this shouldn't happen, but still
        }
    }
}
