...
impl<AnnotatedT> Value<AnnotatedT> {
    ...
    pub fn to_hinted_value(&self, hints: &Hints) -> Result<Option<Value<AnnotatedT>>, ParseError>
    where
    ...
    fn unescape_hint(&self, new_key: &str, key: &Value<AnnotatedT>, value: &Value<AnnotatedT>) -> Self
    where
        AnnotatedT: Annotated + Clone + Default,
    {
        let mut new_map = Map::default();
        new_map.inner.insert(Text::from(new_key).with_annotations_from(key).into(), value.clone());
