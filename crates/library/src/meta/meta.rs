...
impl Meta {
    ...
    fn write_location_debug_representation<WriteT>(
        &self,
        writer: &mut WriteT,
        prefix: &DebugPrefix,
        theme: &Theme,
        ...
        WriteT: io::Write,
    {
        match &self.location {
            Some(location) => {
                write!(writer, " ")?;
                location.write_debug_representation(writer, prefix, theme)
            }
...
pub trait HasMeta: Sized {
    ...
    {
        match self.get_meta() {
            Some(meta) => meta.write_location_debug_representation(writer, prefix, theme),
            None => Ok(()),
        }
    }
}
