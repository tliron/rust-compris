...
impl Debuggable for ConversionError {
        ...
        WriteT: io::Write,
    {
        match self {
            ...
            Self::Casting(casting) => casting.write_debug_representation(writer, prefix, theme),
        }
    }
}
...
impl Debuggable for IncompatibleValueTypeError {
        ...
        WriteT: io::Write,
    {
        write!(
            ...
            self.type_name.style(theme.error),
...
impl Debuggable for CastingError {
    ...
    where
        WriteT: io::Write,
    {
        write!(writer, "{} cannot be cast to a {}", self.value, self.type_name.style(theme.error))
    }
}
...
impl Debuggable for MalformedError {
    ...
    where
        WriteT: io::Write,
    {
        write!(writer, "malformed: {}: {}", self.type_name, self.reason.style(theme.error))
    }
}
