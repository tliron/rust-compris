...
impl Debuggable for IncompatibleValueTypeError {
        ...
        WriteT: io::Write,
    {
        write!(
            ...
            context.theme.error.style(&self.type_name),
...
impl Debuggable for CastingError {
    ...
    where
        WriteT: io::Write,
    {
        write!(writer, "{} cannot be cast to a {}", self.value, context.theme.error.style(&self.type_name))
    }
}
...
impl Debuggable for MalformedError {
    ...
    where
        WriteT: io::Write,
    {
        write!(writer, "malformed: {}: {}", self.type_name, context.theme.error.style(&self.reason))
    }
}
