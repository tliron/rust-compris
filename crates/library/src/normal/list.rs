...
impl Debuggable for List {
        ...
        WriteT: io::Write,
    {
        ...
        for (item, first) in IterateWithFirst::new(self) {
            prefix.conditional_write_with(writer, "- ", first)?;
            item.write_debug_representation(writer, &child_prefix, theme)?;
        }
...
impl List {
        ...
        WriteT: io::Write,
    {
        ...
        for (item, last) in IterateWithLast::new(self) {
            item.write_compact_debug_representation(writer, theme)?;
            if !last {
                write!(writer, "{}", ",".style(theme.delimiter))?;
            }
        }
