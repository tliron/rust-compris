...
impl<R: Read> Reader<R> {
    ...
    fn read_json_for_format(&mut self, xjson: bool) -> Result<Value, ReadError> {
        ...
        read_next_json(&mut reader, &mut value_builder, xjson, self.try_integers, self.try_unsigned_integers)?;
    ...
    match value {
        ...
        ValueType::Number => {
            if try_integers || try_unsigned_integers {
                ...
                if let Some(number) = if try_unsigned_integers { number.parse().ok() } else { None } {
                    value_builder.add(UnsignedInteger::new(number).with_location_option(get_json_location(reader)));
                } else if let Some(number) = if try_integers { number.parse().ok() } else { None } {
        ...
        ValueType::Array => {
            ...
            while reader.has_next()? {
                read_next_json(reader, value_builder, xjson, try_integers, try_unsigned_integers)?;
            }
