...
impl Value {
    ...
    pub fn to_hinted_value(&self, hints: &Hints) -> Result<Option<Value>, ReadError> {
        if let Some((key, value)) = self.get_single_map_entry() {
            if let Value::Text(hint) = key {
                if hint.value == hints.integer {
                    ...
                    for element in &list.value {
                        let entry = element.validate_hinted_list(&hints.map)?;
                        if entry.value.len() == 2 {
                            let mut key = entry.value.get(0).unwrap().clone();
                            if let Some(hinted) = key.to_hinted_value(hints)? {
                                key = hinted;
                            }

                            let mut value = entry.value.get(1).unwrap().clone();
                            if let Some(hinted) = value.to_hinted_value(hints)? {
                                value = hinted;
                            }
