        ...
        WriteT: io::Write,
    {
        match self {
            Self::IncompatibleValueType(incompatible_value_type) => {
                incompatible_value_type.write_debug_representation(writer, prefix, theme)
            }
            Self::Conversion(conversion) => conversion.write_debug_representation(writer, prefix, theme),
            Self::Malformed(malformed) => malformed.write_debug_representation(writer, prefix, theme),
            Self::UnknownKey(invalid_key) => invalid_key.write_debug_representation(writer, prefix, theme),
            Self::MissingRequiredKey(missing_required_key) => {
                missing_required_key.write_debug_representation(writer, prefix, theme)
            }
        }
    }
}
