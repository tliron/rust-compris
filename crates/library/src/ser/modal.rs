...
/// Like [Serialize] but with support for a [SerializationMode]
/// and an embedded [Serializer](ModalSerializer).
...
/// Provides a [Serialize] implementation for a [RecursiveModalSerialize].
...
where
    SerializeModalT: SerializeModalRescursive,
{
    /// Wraps a [RecursiveModalSerialize] with a [RecursiveModalSerializable].
    fn modal<'own>(
        &'own self,
        mode: &'own SerializationMode,
        serializer: &'own ModalSerializer,
