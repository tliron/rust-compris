...
pub fn main() {
    ...
    for user in &users {
        user.print_depiction(&DEFAULT_DEPICTION_CONTEXT);
    }

    if !errors.is_empty() {
        println!();
        errors.annotated_depictions(Some("accumulated errors".into())).print_depiction(&DEFAULT_DEPICTION_CONTEXT);
    }
    ...
    for (index, user) in users.iter().enumerate() {
        ...
        for (field_name, annotation) in annotations {
            ...
            annotation.print_depiction(&DEFAULT_DEPICTION_CONTEXT);
        }
    }
}
