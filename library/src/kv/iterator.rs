...
pub trait KeyValuePairIterator<AnnotatedT> {
    ...
    ///
    /// Also note that the [Result::Err] here is a tuple of the actual error with the value that
    /// caused it, so you likely won't be able to use the `?` operator directly on the result.
    fn next(
        &mut self,
