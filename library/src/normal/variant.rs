...
pub enum Variant<AnnotatedT> {
    /// Undefined. This is not [Null]!
    #[default]
    Undefined,

    /// Null. This is not [Unassigned]!
...
impl<AnnotatedT> Variant<AnnotatedT> {
    ...
    pub fn iterator(&self) -> VariantIterator<AnnotatedT> {
    ...
    ///
    /// Note that the implementation relies on `dyn` to support two different
    /// [KeyValuePairIterator] implementations.
    ...
    pub fn annotated_debuggable(&self) -> AnnotatedDebuggableVariant<AnnotatedT> {
